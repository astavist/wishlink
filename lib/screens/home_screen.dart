...
class _HomeScreenState extends State<HomeScreen> {
      ...
      appBar: AppBar(
        title: GestureDetector(
          ...
          child: Row(
            ...
            children: [
              Image.asset(
                _resolveAppBarAsset(context), // Logo yolunu kendi dosyan�za g顤e g�ncelleyin
                height: 70,
              ),
            ],
          ),
        ...
        leading: IconButton(
          ...
          onPressed: () async {
            // PageView i癟inde Notifications sekmesine ge癟; state g羹ncellemeleri onPageChanged'de yap覺l覺r
  ...
  void _shareActivity(FriendActivity activity) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('${activity.wishItem.name} payla��覺ld覺!'),
  ...
  void _buyNow(FriendActivity activity) {
    if (activity.wishItem.productUrl.isNotEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${activity.wishItem.name} i癟in link bulunamad覺'),
          duration: const Duration(seconds: 2),
        ),
      );
    }
      ...
      if (await canLaunchUrl(uri)) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(const SnackBar(content: Text('Link a癟覺lamad覺')));
        }
      }
    }
  }
